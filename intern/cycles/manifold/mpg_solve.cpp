...
if constexpr (MPG_DEBUG::PARAMS) {
  ...
  /* Validate that the secondary vertex can feasibly reach the light via specular reflection/refraction.
   * Without this check, we generate seeds where Newton cannot converge because the secondary
   * constraint is geometrically impossible to satisfy. */

  // Load secondary geometry to check feasibility
  SpecularSurfaceGeometry secondary_geometry;
  if (!load_surface_geometry(kg, secondary_seed.object, secondary_seed.prim, secondary_geometry)) {
if constexpr (MPG_DEBUG::PARAMS) {
    printf("  trace_secondary_seed: Failed to load secondary geometry\n");
}
    return false;
  }

  // Get secondary vertex position
  const float3 secondary_point = surface_point_from_barycentric(
      secondary_geometry, secondary_seed.bary_u, secondary_seed.bary_v);

  // Get secondary normal
  float3 secondary_normal;
  if (has_smooth_normals) {
    secondary_normal = combine_vertex_normals(
        secondary_geometry, secondary_seed.bary_u, secondary_seed.bary_v);
    if (is_zero(secondary_normal)) {
      secondary_normal = secondary_geometry.normals[0];
    }
  }
  else {
    secondary_normal = secondary_geometry.normals[0];
  }
  secondary_normal = safe_normalize(secondary_normal);
  if (is_zero(secondary_normal)) {
if constexpr (MPG_DEBUG::PARAMS) {
    printf("  trace_secondary_seed: Invalid secondary normal\n");
}
    return false;
  }

  // Get secondary specular parameters
  SpecularParameters secondary_params;
  const bool secondary_is_refraction = get_chaintype_bit(seed.tau_bits, 1);
  secondary_params.is_refraction = secondary_is_refraction;
  secondary_params.base_eta = seed.base_eta;

  // Check if secondary can reach light via specular reflection/refraction
  const float3 dir_to_light_unnormalized = seed.light_sample.P - secondary_point;
  const float dist_to_light = len(dir_to_light_unnormalized);
  if (!(dist_to_light > 1e-4f)) {
if constexpr (MPG_DEBUG::PARAMS) {
    printf("  trace_secondary_seed: Secondary too close to light\n");
}
    return false;
  }
  const float3 dir_to_light = dir_to_light_unnormalized / dist_to_light;

  // Direction from primary to secondary (incoming at secondary)
  const float3 dir_from_primary_unnormalized = secondary_point - primary_point;
  const float dist_primary_to_secondary = len(dir_from_primary_unnormalized);
  if (!(dist_primary_to_secondary > 1e-4f)) {
if constexpr (MPG_DEBUG::PARAMS) {
    printf("  trace_secondary_seed: Primary and secondary too close\n");
}
    return false;
  }
  const float3 dir_from_primary = dir_from_primary_unnormalized / dist_primary_to_secondary;

  // Compute what the specular direction should be at secondary
  bool secondary_tir = false;
  float sec_cos_theta_i, sec_cos_theta_t, sec_eta;
  const float3 secondary_specular_dir = compute_specular(
      dir_from_primary, secondary_normal, secondary_params,
      secondary_tir, sec_cos_theta_i, sec_cos_theta_t, sec_eta);

  if (secondary_tir || is_zero(secondary_specular_dir)) {
if constexpr (MPG_DEBUG::PARAMS) {
    printf("  trace_secondary_seed: Secondary has TIR or invalid specular direction\n");
}
    return false;
  }

  // Check angular alignment between specular direction and light direction
  // For a feasible seed, these should be nearly aligned
  const float alignment = dot(secondary_specular_dir, dir_to_light);
  const float alignment_threshold = 0.7f;  // ~45° tolerance

  if (alignment < alignment_threshold) {
if constexpr (MPG_DEBUG::PARAMS) {
    printf("  trace_secondary_seed: Poor alignment between secondary specular and light direction\n");
    printf("    Secondary specular dir: (%.4f, %.4f, %.4f)\n",
           secondary_specular_dir.x, secondary_specular_dir.y, secondary_specular_dir.z);
    printf("    Direction to light: (%.4f, %.4f, %.4f)\n",
           dir_to_light.x, dir_to_light.y, dir_to_light.z);
    printf("    Alignment (dot product): %.4f (threshold: %.4f)\n",
...
if constexpr (MPG_DEBUG::PARAMS) {
  printf("  trace_secondary_seed: VALIDATION PASSED\n");
  printf("    Secondary specular alignment: %.4f\n", alignment);
}
...
if constexpr (MPG_DEBUG::PARAMS) {
  ...
  for (int iter = 0; iter < options.max_iters; ++iter) {
     ...
     * Mitsuba accepts any geometrically valid step, trusting Newton's method
     * to converge even through non-monotonic residuals. */
    u = new_u;
    v = new_v;
    eval = new_eval;
    params = new_params;
